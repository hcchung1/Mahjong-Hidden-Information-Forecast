...
def main():
    ...
    for epoch in range(EPOCHS): #epoch
        ...
        logger.info(f"Epoch [{epoch+1}/{EPOCHS}], Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Val Accuracy: {val_acc:.4f}")
        if val_acc > max_acc:
            max_acc = val_acc
            # (TODO) Save the model if the validation accuracy is improved
            # raise NotImplementedError
            torch.save(model.state_dict(), 'best_model.pth')
            logger.info(f"Model saved with accuracy: {max_acc:.4f}")
        # raise NotImplementedError
    ...
    logger.info("Plot saved as 'loss.png'")
